...
interface TarotArtFallbackProps {
  card: DrawnTarotCard;
  compact?: boolean;
}
      ...
      <div className="relative rounded-[16px] border-[3px] border-[#e5d0a0] bg-[#efe3c7] p-1 shadow-[0_16px_34px_rgba(0,0,0,0.32)]">
        <div className="overflow-hidden rounded-[10px] border border-[#2b2235] bg-[#f7f0df]">
          {image ? (
            ...
            <TarotArtFallback card={card} compact />
          )}
        </div>
      </div>
    </div>
...
const TarotArtFallback = ({ card, compact = false }: TarotArtFallbackProps) => (
  <div className="relative flex aspect-[7/12] w-full items-center justify-center overflow-hidden bg-[linear-gradient(180deg,#f6ecd6,#eadbb5)]">
    <div className="absolute inset-[8%] rounded-[12px] border border-[#8f6b2d]/25" />
    <div className="absolute inset-x-[18%] top-[16%] h-px bg-[#8f6b2d]/20" />
    <div className="absolute inset-x-[18%] bottom-[16%] h-px bg-[#8f6b2d]/20" />
    <div className="absolute inset-y-[16%] left-[18%] w-px bg-[#8f6b2d]/14" />
    <div className="absolute inset-y-[16%] right-[18%] w-px bg-[#8f6b2d]/14" />
    <div className="flex flex-col items-center gap-3 px-3 text-center text-[#5d4320]">
      <div className={`${compact ? "text-2xl" : "text-4xl"} leading-none`}>{card.glyph}</div>
      <div className={`font-display ${compact ? "text-xs" : "text-sm"} leading-tight`}>
        {card.name}
      </div>
    </div>
  </div>
...
const Tarot = () => {
  ...
  useEffect(() => {
    ...
    fetchProfileContext(user.id).then(({ data, error }) => {
        ...
        if (error) {
          console.error("Failed to load profile for tarot:", error);
        }
