  ...
  const cardLines = tarot.cards
    .map((card) => {
      ...
      const keywords = Array.isArray(card.keywords)
        ? card.keywords.filter((keyword) => typeof keyword === "string" && keyword.trim()).join(", ")
        : "";
      const meaning =
...
export default async (request) => {
  ...
  try {
    const { question, profile, history, tarot } = await request.json();
    ...
    if (ageRestrictionError) {
      return new Response(JSON.stringify({ error: ageRestrictionError }), {
        status: 403,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }
    ...
    if (!response.ok) {
      ...
      const message = formatProviderError(response.status, data);
